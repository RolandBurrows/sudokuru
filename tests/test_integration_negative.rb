...
describe "sudokuru_negatives" do
  ...
  it "should detect that puzzle file has duplicate characters in a box" do
    ARGV[0] = "./puzzles/9x9.txt"
    ARGV[1] = "./puzzles/9x9boxb.txt"
    output = capture_stdout { load "sudokuru.rb" }
    output.must_include "ERROR"
    output.must_include "Box A (-28-16-13) contains duplicate values. Please fix and rerun."
  end
